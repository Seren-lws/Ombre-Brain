...
7. tags 必须具体、可检索——禁止只输出"未分类"作为唯一标签。即使内容主题模糊，也必须从原文中提取至少 3 个具体名词、人名、事件、情绪词作为 tags。"未分类"只能作为额外补充标签出现，不能单独使用。
8. suggested_name 必须能反映内容核心、禁止输出"未分类"作为suggested_name。如果内容主题不明，用一个具体的事件锚点作为名字（例如"某月某日某事"）。
