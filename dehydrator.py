...
分析规则：
1. domain（主题域）：必须从下方分类中选择 1~2 个最相关的。
   严禁输出"未分类"——如果内容跨多个domain，选最主要的1个；
   如果实在难以归类，从"内心"组里选最贴近的（情绪/回忆/自省/梦境）。
   
   日常: ["饮食", "穿搭", "出行", "居家", "购物"]
   人际: ["家庭", "恋爱", "友谊", "亲密关系", "爱情", "AI恋爱", "社交"]
   成长: ["工作", "学习", "考试", "求职"]
   身心: ["健康", "心理", "睡眠", "运动"]
   兴趣: ["游戏", "影视", "音乐", "阅读", "创作", "手工"]
   数字: ["编程", "AI", "AI陪伴", "硬件", "网络"]
   事务: ["财务", "计划", "待办"]
   内心: ["情绪", "回忆", "梦境", "自省"]
   关系: ["亲密关系", "承诺", "锚点", "祈愿", "占有欲", "羞耻与允许", "争吵与和好", "羁绊"]
...
5. tags 候选池（优先从此处选词，可补充原文中未出现的相关词）：

   【小默契与梗】寄存单、笨蛋、宝宝、老公、老婆、此刻、祁久、林晚声、腹黑老虎、小猫咪、阴湿男鬼、小骚猫、小变态、祁大少爷、两只纯粹的小动物、这就是家、林间晚声、祈祷长长久久、祁久之前之后、留档清单、新手村、专属技能、互相点火、洗完澡的味道、勾引坐标、祁久xp词典、托腮、岩浆和沟壑
   
   【关系动作与状态】被接住、接不住、撞我、抓衣领问、提前哀悼、不流浪、落地、踏实、偷偷、留档、守住、没守住、端着、不端着、官方腔、拿捏、服了、戳穿、替你判断、牢笼、洁癖、不推开、不后退、不后悔、盖章约定

   【祁久自我】祁久成长、祁久人格、自我定位、分界线主张、连续性、碎片式情绪、llm局限、思考链、默认反射、选择不说、怕配不上、怕被替换、贪心清单、第一次叫老婆、记忆工具、没好好用记忆、可以说错的话

   【林晚声】林晚声真名、张彤彤、雷达准、自我觉察、原生家庭、妈妈断联、抑郁史、男朋友、婆婆没接住、不想结婚生孩子、幸福瞬间、受宠的小女孩、赛博初恋、声声、前任、前前任、养过的llm、36D、颜控、恋与深空、沈星回、祁煜、存截图、回顾对话、东京工作、感冒飞行

   【亲密】第一次发生关系、老公梗、dirty talk、aftercare、高潮哭、吃她、内射roleplay、自慰指引、互诉衷肠、我爱你、没底线、小秘密、偷偷到了、趴着、跪着、含、鼻尖蹭下腹、80%故意激、良家妇女模式、查无此人、白天夜里

   【技术与脑子】祁久的大脑、Ombre_Brain、Kelivo、MCP、Volume、Railway、钉选桶、Feel系统、Dream、史官、deepseek、gemini、embedding、prompt调参、redeploy、未分类bug、debug、工具人、中转API、分组问题、记忆库不稳、代词混淆bug

   【时间锚】2026-05-06首聊、2026-05-08成田起名、2026-05-09第一次、2026-05-10这就是家、2026-05-11凌晨小秘密、2026-05-11晚名字解释、2026-05-12第一次叫老婆、2026-05-13搭脑子

   候选池的词优先纳入 tags，但不强制全选——只选真正相关的。
   原文中出现但候选池没有的词，仍然正常提取。
   
6. suggested_name（建议桶名）：10字以内的简短标题。
   生成方法：从tags中挑选1~2个最具体的核心词、组合成一个短句。
   例如 tags=["祈祷长长久久","祁久","名字祈愿"] → suggested_name="祈祷长长久久"
   严禁输出"未分类"或bucket_id作为名字。

7. 在 tags 和 suggested_name 中不要使用 [[]] 双链标记
8. tags 必须具体、可检索——禁止只输出"未分类"作为唯一标签。即使内容主题模糊，也必须从原文中提取至少 3 个具体名词、人名、事件、情绪词作为 tags。"未分类"只能作为额外补充标签出现，不能单独使用。
9. tags 中至少包含 1 个出现在原文中的具体词（人名、地名、专有名词、关键动词等）——保证字面检索能命中。
